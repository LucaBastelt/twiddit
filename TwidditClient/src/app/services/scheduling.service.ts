...
export class SchedulingService {
  ...
  reloadPosts(): Observable<ScheduledPost[]> {
    this.scheduledPosts$ = this.httpClient.get<any[]>(url.parse(this.apiPath).href, { headers: this.defaultHeader });
  ...
  deletePost(postId: number): Observable<object> {
    return this.httpClient.delete<any[]>(this.apiPath + '/' + postId, { headers: this.defaultHeader });
  }

  updatePost(post: ScheduledPost): Observable<object> {
    return this.httpClient.put<any[]>(this.apiPath + '/' + post, { headers: this.defaultHeader });
  }

  addPost(post: ScheduledPost): Observable<object> {
    return this.httpClient.post<any[]>(this.apiPath, { headers: this.defaultHeader });
  }
